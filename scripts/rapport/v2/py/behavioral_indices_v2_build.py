...
TAS_V2_FEATURE_SIGNS = [("gaze_convergence_ratio", +1)]
TMS_V2_FEATURES = [
...
def _build_indices_v2_table(base_bundle: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    # Priorité colonne directionnelle, fallback legacy
    conv_col = "gaze_convergence_ratio" if "gaze_convergence_ratio" in analysis_df.columns else "shared_obj_ratio"
    indices_v2["gaze_convergence_ratio_z"] = zscore_series(analysis_df[conv_col])
    indices_v2["shared_obj_ratio_z"] = indices_v2["gaze_convergence_ratio_z"]  # alias legacy
    indices_v2["I_TAS_b_v2"] = indices_v2["gaze_convergence_ratio_z"]
