...
def load_gaze_group(path: str) -> pd.DataFrame:
    ...
    numeric_cols = [
        ...
        "mutual_gaze_n_episodes_per_s", "mutual_gaze_dur_total_ratio",
    ...
    if "shared_obj_ratio" not in df.columns and "group_shared_obj_pct_mean" in df.columns:
        df["shared_obj_ratio"] = pd.to_numeric(df["group_shared_obj_pct_mean"], errors="coerce")
    if "shared_obj_dur_mean_s" not in df.columns and "group_shared_episode_median_s" in df.columns:
        df["shared_obj_dur_mean_s"] = pd.to_numeric(df["group_shared_episode_median_s"], errors="coerce")
    if "shared_obj_dur_mean_s" in df.columns:
        df["gaze_shared_obj_episode_dur_mean_s"] = df["shared_obj_dur_mean_s"]

    if "gaze_shared_obj_episode_dur_mean_s" in df.columns:
        df["log_gaze_shared_obj_episode_dur_mean_s"] = np.log1p(
            pd.to_numeric(df["gaze_shared_obj_episode_dur_mean_s"], errors="coerce").clip(lower=0)
        )

    if "gaze_entropy_mean_participants" in df.columns:
        df["gaze_focus_proxy"] = 1.0 - pd.to_numeric(df["gaze_entropy_mean_participants"], errors="coerce")

    keep = [
        "group_id", "condition", "scenario", "timepoint",
        "interaction_dur_s",
        "shared_obj_ratio",
        "shared_obj_n_episodes", "shared_obj_dur_total_s", "shared_obj_dur_mean_s",
        "gaze_shared_obj_episode_dur_mean_s", "log_gaze_shared_obj_episode_dur_mean_s",
        "shared_obj_dur_median_s", "shared_obj_dur_q25_s", "shared_obj_dur_q75_s", "shared_obj_dur_iqr_s",
        "shared_obj_n_episodes_per_s", "shared_obj_dur_total_ratio",
        "mutual_gaze_ratio_mean_pairs",
        "mutual_gaze_n_episodes_sum_pairs", "mutual_gaze_dur_total_s_sum_pairs",
        "mutual_gaze_n_episodes_per_s", "mutual_gaze_dur_total_ratio",
        "gaze_entropy_mean_participants",
        "gaze_focus_proxy",
        "gaze_to_speaker_ratio",
...
def load_gaze_pair(path: str) -> pd.DataFrame:
    ...
    if "pair_shared_obj_ratio" not in df.columns and "pair_shared_obj_pct_mean" in df.columns:
        df["pair_shared_obj_ratio"] = pd.to_numeric(df["pair_shared_obj_pct_mean"], errors="coerce")
    ...
    grp = df.groupby(["group_id", "condition", "scenario", "timepoint"], dropna=False).agg(
        pair_shared_obj_ratio_mean=("pair_shared_obj_ratio", "mean"),
        pair_shared_obj_dur_mean_s_mean=("pair_shared_obj_dur_mean_s", "mean"),
        pair_mutual_gaze_ratio_mean=("pair_mutual_gaze_ratio", "mean"),
        pair_mutual_gaze_dur_mean_s_mean=("pair_mutual_gaze_dur_mean_s", "mean"),
    ).reset_index()
...
def _apply_reference_duration_and_rates(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "shared_obj_n_episodes" in df.columns:
    ...
    if n_ep is not None:
        df["shared_obj_episode_rate_per_min_ref"] = n_ep / (dur_min + 1e-6)

    if "shared_obj_dur_total_s" in df.columns:
        tot_s = pd.to_numeric(df["shared_obj_dur_total_s"], errors="coerce")
        df["shared_obj_dur_total_ratio_ref"] = tot_s / (df["interaction_dur_s_ref"] + 1e-6)
    ...
    if "mutual_gaze_n_episodes_sum_pairs" in df.columns:
    ...
    if "mutual_gaze_dur_total_s_sum_pairs" in df.columns:
...
def add_final_feature_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["gaze_mutual_gaze_ratio"] = first_valid_series(
        df,
        ["mutual_gaze_ratio_mean_pairs", "pair_mutual_gaze_ratio_mean", "mutual_gaze_dur_total_ratio_ref"],
    )
    ...
    df["gaze_entropy"] = first_valid_series(df, ["gaze_entropy_mean_participants"])
    df["gaze_entropy_source"] = first_valid_source(df, ["gaze_entropy_mean_participants"])
...
def compute_composites(df: pd.DataFrame, z_by: Optional[List[str]] = None, w: Weights = Weights()) -> pd.DataFrame:
    ...
    if "shared_obj_ratio" in df.columns and "mutual_gaze_ratio_mean_pairs" in df.columns:
        df["gaze_joint_attention_idx_raw"] = (
            pd.to_numeric(df["shared_obj_ratio"], errors="coerce") +
            pd.to_numeric(df["mutual_gaze_ratio_mean_pairs"], errors="coerce")
        ) / 2.0
    ...
    if "shared_obj_n_episodes" in df.columns and "shared_obj_dur_total_s" in df.columns:
        df["shared_obj_episode_density_raw"] = ratio(
            pd.to_numeric(df["shared_obj_n_episodes"], errors="coerce"),
            pd.to_numeric(df["shared_obj_dur_total_s"], errors="coerce"),
        )
    ...
    if "z_shared_obj_ratio" in df.columns:
        coord_terms.append(w.gaze_shared_obj * df["z_shared_obj_ratio"])
    if "z_mutual_gaze_ratio_mean_pairs" in df.columns:
    ...
    # MOD-9 : poids egaux (-1.0) assumes en l'absence de validation empirique (ancien poids = -0.10 via w.gaze_entropy_penalty)
    gaze_terms_old = []
    ...
    # MOD-10 : z_mutual_gaze_ratio_mean_pairs retire — terme structurellement nul en VR
    # (variance=0 pour les 11 groupes VR, audit 2026-05). Formule = nanmean(z_shared_obj_ratio, -z_gaze_entropy).
    gaze_terms = []
    for c in ["z_shared_obj_ratio"]:
        if c in df.columns:
            gaze_terms.append(df[c])
    if "z_gaze_entropy_mean_participants" in df.columns:
        gaze_terms.append(-1.0 * df["z_gaze_entropy_mean_participants"])
...
def _drop_merge_suffixes(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    pairs = [
        ("interaction_dur_s_x", "interaction_dur_s_y", "interaction_dur_s"),
        ("interaction_duration_s_x", "interaction_duration_s_y", "interaction_duration_s"),
    ]

    for cx, cy, out in pairs:
        if cx in df.columns or cy in df.columns:
            sx = pd.to_numeric(df[cx], errors="coerce") if cx in df.columns else None
            sy = pd.to_numeric(df[cy], errors="coerce") if cy in df.columns else None
...
def make_compact_output(df: pd.DataFrame) -> pd.DataFrame:
    compact_cols = [
        ...
        "gaze_to_speaker_ratio_final_source",
        ...
        "n_missing_gaze_core",
        "n_missing_face_core",
        # MOD-8 : z-scores affect alignment conserves dans sortie compacte
        "z_joy_tri_occupancy",
        "z_joy_sync_jaccard_mean",
        "z_sad_tri_occupancy",
        "z_sad_sync_jaccard_mean",
        # MOD-9 : z-scores gaze conserves dans sortie compacte
        "z_shared_obj_ratio",
        "z_mutual_gaze_ratio_mean_pairs",
        "z_gaze_entropy_mean_participants",
    ]
...
def main():
    ...
    core_gaze = [c for c in ["shared_obj_ratio", "mutual_gaze_ratio_mean_pairs", "gaze_entropy_mean_participants"] if c in merged.columns]
