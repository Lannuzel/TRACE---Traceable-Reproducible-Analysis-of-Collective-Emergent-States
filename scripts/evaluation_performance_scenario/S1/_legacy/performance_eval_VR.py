...
What's new in v3:
- Uses Hungarian assignment (global optimum) if SciPy is available; else falls back to greedy NN.
- Accepts participant file or glob pattern (picks newest by default).
- Computes requested metrics:
    * count_diff = n_placed - n_expected (and abs_count_diff)
    * sum_distance_m = sum of distances over matched pairs
    * duration_s = max timestamp found (free-form logs or structured)
- Optional overlay figure with:
    * solution vs participant scatter
    * thin lines for matched pairs
    * optional tolerance circles
    * a small metrics box on the plot
- Clean, CLI-friendly API
...
def extract_duration_seconds(df_structured: pd.DataFrame, df_freeform: pd.DataFrame | None) -> float | None:
    if df_freeform is not None and not df_freeform.empty and "time" in df_freeform.columns:
        return float(df_freeform["time"].max())
...
def hungarian_assignment(S: np.ndarray, P: np.ndarray) -> List[Tuple[int, int, float]]:
    ...
    try:
        ...
        # Build square cost matrix (pad with large costs if sizes differ)
        nS, nP = S.shape[0], P.shape[0]
        D = np.sqrt(((S[:, None, :] - P[None, :, :]) ** 2).sum(axis=2))
        if nS == nP:
            row_ind, col_ind = linear_sum_assignment(D)
        else:
            # Pad to square with a large penalty to avoid spurious matches
            n = max(nS, nP)
...
# ------------------------------ Core ------------------------------
def compute_performance(solution_path: str,
    ...
    if set(axes).issubset(par.columns):
        ...
        par_points["name"] = par[name_col[0]].astype(str) if name_col else ""
        parsed_freeform = None
    else:
        parsed_freeform = parse_freeform_positions(par)
        if parsed_freeform.empty:
            raise ValueError("Participant file could not be parsed (neither structured nor free-form).")
        par_points = parsed_freeform[list(axes) + ["name"]].copy() if "name" in parsed_freeform.columns else parsed_freeform.assign(name="")[list(axes)+["name"]]
        used_freeform = True
    ...
    for si, pi, dist in matches:
        sp = sol_points.iloc[si]; pp = par_points.iloc[pi]
        rows.append({
    ...
    return report, summary, par_points, sol_points
...
def draw_tolerance_circles(ax, sol_points: pd.DataFrame, axes=("PositionX","PositionZ"), radius=0.25):
    try:
        ...
        for _, r in sol_points.iterrows():
            circ = patches.Circle((float(r[axes[0]]), float(r[axes[1]])), radius=radius, fill=False, alpha=0.25, linewidth=0.8)
            ax.add_patch(circ)
    except Exception:
        pass
...
# ------------------------------ CLI ------------------------------
def main():
    ap = argparse.ArgumentParser(description="Compute participant performance (count diff, sum distance, duration) with Hungarian assignment.")
    ap.add_argument("--solution", required=True, help="Solution file (.xlsx/.csv) with PositionX, PositionZ")
    ap.add_argument("--participant", required=True, help="Participant file path OR glob (e.g., path\\*_ChairePositionData.csv)")
    ap.add_argument("--tolerance", type=float, default=0.25, help="Tolerance in meters (for within_tolerance + optional circles)")
    ap.add_argument("--axes", default="PositionX,PositionZ", help="Comma-separated axes (default: plan X,Z)")
    ...
    report, summary, par_points, sol_points = compute_performance(
        args.solution, args.participant, tolerance_m=args.tolerance, axes=axes, pick=args.pick, assignment=args.assignment
    )
    ...
    if args.plot:
        # reconstruct matches for plotting using chosen assignment
        S = sol_points[list(axes)].to_numpy(float)
        P = par_points[list(axes)].to_numpy(float)
        if args.assignment == "hungarian":
            matches = hungarian_assignment(S, P)
        else:
            matches = greedy_nearest_neighbor(S, P)
