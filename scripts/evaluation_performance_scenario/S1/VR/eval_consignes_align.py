...
Tolérances d’alignement déduites automatiquement des espacements inférés.
...
def read_table(path: str) -> pd.DataFrame:
    ...
    except Exception: return pd.read_csv(path, sep=None, engine="python")
...
def auto_calibrate(P_df: pd.DataFrame, S_df: pd.DataFrame, axes=("PositionX","PositionZ"), tol_level: float = 0.25) -> Dict[str,Any]:
    ...
    Identique à la version PC pour cohérence méthodologique.
    """
    ...
    if cx_P.size == 0 or cx_S.size == 0:
        ...
        tx = float(np.min(cx_S) - np.min(cx_P))

    if cz_P.size == 0 or cz_S.size == 0:
        tz = float(np.mean(Sz) - np.mean(Pz))
    else:
        tz = float(np.min(cz_S) - np.min(cz_P))
...
def main():
    ...
    ap.add_argument("--no_calib", action="store_true", help="Désactiver la calibration P->S")
    ap.add_argument("--allow_scale", action="store_true",
                help="Autoriser la mise à l'échelle dans la calibration. Par défaut: translation seule.")
    ...
    # calibration optionnelle (translation seule par défaut, cohérent avec PC)
    ...
    # tolérance d'accrochage aux rangées : adaptée au pas (cohérent avec version PC)
