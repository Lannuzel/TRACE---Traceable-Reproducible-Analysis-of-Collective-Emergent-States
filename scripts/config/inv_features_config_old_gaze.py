...
_EXCLUDED_EXACT_NAMES = frozenset({
    ...
    "max_speech_ratio",
    # Gaze : préférer shared_obj_episode_rate_per_min_ref
...
PRUNING_PROTECTED_PAIRS: frozenset[frozenset[str]] = frozenset({
    ...
    frozenset({"gaze_entropy_mean_participants", "gaze_shared_visual_attention_ratio"}),
...
MODALITY_PREFIX: dict[str, list[str]] = {
    ...
    "gaze": [
        "gaze_", "mutual_gaze", "shared_obj", "pair_mutual_gaze",
        "pair_shared_obj", "transition_prob_gaze",
    ],
}
...
INV_FEATURES: dict[str, dict[str, Any]] = {
    ...
    "face_facial_synchrony": {
        ...
        "calc_method": (
            ...
            "{au6_active_pct, au12_active_pct, au15_active_pct, au17_active_pct, "
            "au6_au12_coactive_pct, au15_au17_coactive_pct} : "
            "r_(ab,k) = Pearson(AU_k_a(fenêtre), AU_k_b(fenêtre)) ; "
            "au_sync_pearson_mean = mean(r_(ab,k)) sur 6 métriques × 3 paires. "
    ...
    "au_sync_jaccard_mean": {
        ...
        "calc_method": (
            "Moyenne des scores de Jaccard sur 6 colonnes AU × 3 paires : "
    ...
    "au_sync_pearson_mean": {
        ...
        "calc_method": (
            "Moyenne des corrélations de Pearson sur 6 métriques AU × 3 paires "
    ...
    "gaze_entropy_mean_participants": {
        ...
        "description": "Entropie moyenne du regard",
        "calc_method": (
            ...
            "valeur de groupe = mean(entropies individuelles) "
            "— analyze_gaze.py"
        ),
    },
    "shared_obj_ratio": {
    ...
    "gaze_shared_obj_episode_dur_mean_s": {
        ...
        "description": "Durée moyenne par épisode d'objet partagé",
        "calc_method": (
            "dur_total_s / n_episodes via episodes_from_bool sur la grille "
            "de co-fixation ; durée moyenne d'un épisode de co-regard partagé, en secondes "
            "— analyze_gaze.py (shared_obj_dur_mean_s)"
        ),
        "reason": "Capte la persistance des épisodes d'attention partagée, indépendamment de leur fréquence",
    },
    "log_gaze_shared_obj_episode_dur_mean_s": {
        "family": "gaze",
        "priority": 31,
        ...
        "report_preferred": True,
        "reference": "Biancardi et al. (2023) — J. Multimodal User Interfaces",
        "description": "log1p(durée moyenne par épisode d'objet partagé) — pour régression",
        "calc_method": (
            "log1p(gaze_shared_obj_episode_dur_mean_s) — compute_high_level_features.py. "
            "Réduit l'asymétrie droite de la distribution brute."
        ),
        "reason": "Distribution brute fortement asymétrique ; log-transform améliore la linéarité pour la régression",
    },
    ...
    "gaze_shared_visual_attention_ratio": {
        ...
        "core": True,
        "core_hl": True,
        "redundant_with": ["shared_obj_ratio", "gaze_joint_attention_idx_raw"],
        "drop_if_redundant": False,
        "regression_preferred": True,
        "report_preferred": True,
        "reference": "Richardson & Dale (2005) — Psychological Science",
        "description": "Attention visuelle partagée (chaîne de fallback : shared_obj_ratio en source primaire)",
        "calc_method": (
            "first_valid_series([shared_obj_ratio, shared_obj_dur_total_ratio_ref, "
            "pair_shared_obj_ratio_mean]) — premier résultat non-NaN de la chaîne ; "
            "audit 2026-05 : 11/11 groupes VR utilisent shared_obj_ratio (source primaire, r=0.800 avec shared_obj_dur_total_ratio_ref) "
            "— compute_high_level_features.py"
        ),
        "reason": (
    ...
    "gaze_entropy": {
        ...
        "priority": 34,
        "core": False,
        "core_hl": False,  # core_hl=False : drop_if_redundant=True incompatible avec CORE_HL
        "redundant_with": ["gaze_entropy_mean_participants"],
        "drop_if_redundant": True,
        "description": "Dispersion du regard",
        "calc_method": (
            "first_valid_series([gaze_entropy_mean_participants]) — alias direct "
            "via first_valid_series dans le pipeline HLF "
            "— compute_high_level_features.py"
        ),
        "reason": "|r|≈0.92 avec gaze_entropy_mean_participants",
    },
    "gaze_attention_coordination_idx": {
        "family": "gaze",
        "priority": 35,
        ...
        "report_preferred": True,
        "reference": "Contribution originale (pas de référence directe)",
        "description": "Coordination de l'attention (composite : objet partagé − entropie regard)",
        "calc_method": (
            "nanmean([z_shared_obj_ratio, −1.0 × z_gaze_entropy_mean_participants]) "
            "à poids égaux (1/2 chacun) "
            "— compute_high_level_features.py. "
            "MOD-10 (2026-05) : z_mutual_gaze_ratio_mean_pairs retiré — terme structurellement "
            "nul en VR (variance=0 pour les 11 groupes, audit 2026-05-18) ; "
            "les valeurs existantes ne changent pas car ce terme valait déjà 0."
        ),
    },
    "gaze_mutual_gaze_ratio": {
        ...
        "priority": 36,
        "core": True,
        "core_hl": True,  # valide : analyses réalisées en VR uniquement
        "redundant_with": [],
        "drop_if_redundant": False,
        "description": "Ratio global de regard mutuel",
        "calc_method": (
            "first_valid_series([mutual_gaze_ratio_mean_pairs, pair_mutual_gaze_ratio_mean, "
            "mutual_gaze_dur_total_ratio_ref]) — premier résultat non-NaN "
            "— compute_high_level_features.py"
        ),
